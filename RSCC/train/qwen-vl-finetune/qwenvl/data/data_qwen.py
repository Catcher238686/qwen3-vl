...
def preprocess_qwen_2_visual(
    ...
    grid_thw = grid_thw or []  # Ensure grid_thw is always a list
    input_ids, targets = [], []
...
class LazySupervisedDataset(Dataset):
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0] or "images" in sources[0]:
            ...
            # Get image files from either 'image' or 'images' field
            ...
            # Validation check for visual tags
            conversation = sources[0]["conversations"]
            num_visual_tags = sum(
                conv["value"].count("<image>")
                for conv in conversation  # Changed from simple existence check
            )
